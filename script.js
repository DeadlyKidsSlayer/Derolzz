...
soalUjian.forEach((soal) => {
  ...
  li.textContent = soal.nomor; // Hanya tampilkan angka nomor soal
...
function tampilkanSoal(nomor) {
  ...
  semuaNomor.forEach((nomorSoal) => nomorSoal.classList.remove("active", "answered"));
  ...
  // Jika soal sudah dijawab, tambahkan kelas "answered"
  if (jawabanPengguna[nomor]) {
    nomorAktif.classList.add("answered");
  }
  ...
  pilihanJawaban.forEach((pilihan) => {
    pilihan.addEventListener("change", () => {
      ...
      // Update warna kotak nomor soal
      const nomorSoal = document.querySelector(`#daftar-nomor-soal li:nth-child(${soal.nomor})`);
...
ulangiUjianButton.addEventListener("click", () => {
  ...
  backsound.play() // Putar musik
    .then(() => {
      console.log("Musik berhasil diputar ulang.");
    })
    .catch((error) => {
  ...
  // Reset warna nomor soal
  const semuaNomor = document.querySelectorAll("#daftar-nomor-soal li");
  semuaNomor.forEach((nomorSoal) => nomorSoal.classList.remove("answered"));
});